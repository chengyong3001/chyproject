...
class PytablesBitextFetcher(threading.Thread):
    ...
    def run(self):
        ...
        target_table = tables.openFile(diter.target_file, 'r', driver=driver)
        target_data, target_index = (target_table.getNode(diter.table_name),
            target_table.getNode(diter.index_name))

        target_table_1 = tables.openFile(diter.target_file_1, 'r', driver=driver)
        target_data_1, target_index_1 = (target_table_1.getNode(diter.table_name),
            target_table_1.getNode(diter.index_name))


        source_table = tables.openFile(diter.source_file, 'r', driver=driver)
        source_data, source_index = (source_table.getNode(diter.table_name),
            source_table.getNode(diter.index_name))

        source_table_1 = tables.openFile(diter.source_file_1, 'r', driver=driver)
        source_data_1, source_index_1 = (source_table_1.getNode(diter.table_name),
            source_table_1.getNode(diter.index_name))

        assert source_index.shape[0] == target_index.shape[0]
        assert source_index_1.shape[0] == target_index_1.shape[0]
        data_len = source_index.shape[0]
        data_len_1 = source_index.shape[0]

        offset = self.start_offset
        offset_1 = self.start_offset
        #force it to be -1
        offset = -1
        ...
        if offset == -1:
            ...
            if diter.shuffle:
                ...
                offset_1 = np.random.randint(data_len_1)
        ...
        while not diter.exit_flag:
            ...
            while len(source_sents_1) < diter.batch_size:
                if offset_1 == data_len_1:
                    if diter.use_infinite_loop:
                        offset_1 = 0
                    else:
                        last_batch = True
                ...
                slen, spos = source_index_1[offset_1]['length'], source_index_1[offset_1]['pos']
                tlen, tpos = target_index_1[offset_1]['length'], target_index_1[offset_1]['pos']
                offset_1 += 1

                if slen > diter.max_len or tlen > diter.max_len:
                    continue
                source_sents_1.append(source_data_1[spos:spos + slen].astype(diter.dtype))
                target_sents_1.append(target_data_1[tpos:tpos + tlen].astype(diter.dtype))

            if len(source_sents):
                diter.queue.put([int(offset), source_sents, target_sents, int(offset_1), source_sents_1, target_sents_1])
            if last_batch:
...
class PytablesBitextIterator(object):
    ...
    def next(self):
        ...
        self.next_offset_1 = batch[3]
        return batch[1], batch[2], batch[4], batch[5]
