...
from groundhog.layers import\
        ...
        DropOp,\
        Concatenate
from groundhog.models import LM_Model, Multi_LM_Model
...
def get_batch_iterator(state):
    ...
    class Iterator(PytablesBitextIterator):
        ...
        def get_homogenous_batch_iter(self):
            while True:
                ...
                x_1 = numpy.asarray(list(itertools.chain(*map(operator.itemgetter(2), data))))
                y_1 = numpy.asarray(list(itertools.chain(*map(operator.itemgetter(3), data))))
                lens_1 = numpy.asarray([map(len, x_1), map(len, y_1)])
                order_1 = numpy.argsort(lens_1.max(axis=0)) if state['sort_k_batches'] > 1 \
                        else numpy.arange(len(x_1))

                for k in range(k_batches):
                    indices = order[k * batch_size:(k + 1) * batch_size]
                    batch = create_padded_batch(state, [x[indices]], [y[indices]],
                            return_dict=True)
                    indices_1 = order_1[k * batch_size:(k + 1) * batch_size]
                    batch_1 = create_padded_batch(state, [x_1[indices_1]], [y_1[indices_1]],
                            return_dict=True)
                    batch['x_1'] = batch_1['x']
                    batch['x_mask_1'] = batch_1['x_mask']
                    batch['y_1'] = batch_1['y']
                    batch['y_mask_1'] = batch_1['y_mask']
                    if batch:
                        yield batch
    ...
    train_data = Iterator(
        ...
        source_file_1=state['source'][1],
        target_file_1=state['target'][1],
        can_fit=False,
        queue_size=1000,
...
class RecurrentLayerWithSearch(Layer):
    ...
    def step_fprop(self,
        ...
        p_from_h = ReplicateLayer(source_len)(utils.dot(state_before, B_hp)).out        #B_hp: Wa

        # Form projection to the tanh layer from the source annotation.
        if not p_from_c:
            p_from_c =  utils.dot(c, A_cp).reshape((source_len, source_num, dim))       #A_cp: Ua, c:hj

        # Sum projections - broadcasting happens at the dimension 1.
        p = p_from_h + p_from_c

        # Apply non-linearity and project to energy.
        energy = TT.exp(utils.dot(TT.tanh(p), D_pe)).reshape((source_len, target_num))      #D_pe:Va
        if c_mask:
        ...
        ctx = (c * probs.dimshuffle(0, 1, 'x')).sum(axis=0)         #ctx: ci

        state_below += self.c_inputer(ctx).out          #state_below: E(yi-1))), c_inputer: C
        reseter_below += self.c_reseter(ctx).out        #reseter_below: WrE(yi-1), c_reseter: Cr
        updater_below += self.c_updater(ctx).out        #updater_below: WzE(yi-1), c_updater: Cz

        # Reset gate:
        # optionally reset the hidden state.
        reseter = self.reseter_activation(TT.dot(state_before, R_hh) +      #R_hh: Ur
                reseter_below)
        reseted_state_before = reseter * state_before

        # Feed the input to obtain potential new state.
        preactiv = TT.dot(reseted_state_before, W_hh) + state_below         #W_hh: U
        h = self.activation(preactiv)                                       #h: si

        # Update gate:
        # optionally reject the potential new state and use the new one.
        updater = self.updater_activation(TT.dot(state_before, G_hh) +      #G_hh: Uz
                updater_below)
    ...
    def fprop(self,
        ...
        p_from_c =  utils.dot(c, self.A_cp).reshape(        #A_cp: Ua
        ...
        rval, updates = theano.scan(fn,
                        ...
                        n_steps=nsteps)
        if return_alignment:
            rval[2] = rval[2].reshape((rval[2].shape[0], c.shape[0], batch_size))
        self.out = rval
        self.rval = rval
...
class EncoderDecoderBase(object):
    ...
    def _create_embedding_layers(self, shared_embedding=None):
...
class Encoder(EncoderDecoderBase):
    ...
    def create_layers(self, shared_X=None):
        ...
        self._create_embedding_layers(shared_embedding=shared_X)
...
class Decoder(EncoderDecoderBase):
    ...
    def create_layers(self,shared_Y=None):
        ...
        self._create_embedding_layers(shared_embedding=shared_Y)
        self._create_transition_layers()
        self._create_inter_level_layers()
    ...
    def build_decoder(self, c, y,
        ...
        for level in range(self.num_levels):
            ...
            add_kwargs = (dict(state_before=init_states[level],flatten=False)
                        if mode != Decoder.EVALUATION
            ...
            if self.state['search']:
                if self.compute_alignment:
                    ...
                    #alignment shape (target_seq_len, source_len, batch_size)
                    h, ctx, alignment = result

                    #added by chengyong
                    self.alignment_layer = alignment
                    ####
        ...
        if mode == Decoder.SAMPLING:
            ...
            ###########################################
            # by He Wei
            #self.align_matrix =  self.transitions[0].align_matrix
    ...
    def build_next_probs_predictor(self, c, step_num, y, init_states):
        ######################################################
        # by He Wei
        #_result = self.build_decoder(c, y, mode=Decoder.BEAM_SEARCH,
        #        given_init_states=init_states, step_num=step_num)
        #if self.compute_alignment:
        #    self.align = _result[1]
        #else:
        #    self.align = TT.zeros((1,))
    ...
    ######################################################
    # by He Wei
    def build_next_align_predictor(self):
        return TT.argmax(self.align)
    ######################################################
...
class RNNEncoderDecoder(object):
    ...
    def __init__(self, state, rng, prefix = "",
            skip_init=False,
        ...
        self.state = state
        self.rng = rng
        self.prefix = prefix #added by chengyong
        self.name = prefix #added by chengyong
        self.skip_init = skip_init 
        self.compute_alignment = compute_alignment

    def build(self, x=None, x_mask=None, y=None, y_mask=None, shared_X=None, shared_Y=None):
        ...
        self.encoder.create_layers(shared_X=shared_X)
        ...
        self.backward_encoder = Encoder(self.state, self.rng,
                prefix= self.prefix+"back_enc",
                skip_init=self.skip_init)
        self.backward_encoder.create_layers(shared_X=shared_X)
        ...
        self.decoder = Decoder(self.state, self.rng, prefix = self.prefix +"dec",
                skip_init=self.skip_init, compute_alignment=self.compute_alignment)
        self.decoder.create_layers(shared_Y=shared_Y)
        
        logger.debug("Create source context")
        self.source_ctx = Concatenate(axis=2)(*training_c_components)
        
        logger.debug("Build log-likelihood computation graph")
        self.predictions, self.alignment = self.decoder.build_decoder(
                c=self.source_ctx, c_mask=self.x_mask,
                y=self.y, y_mask=self.y_mask)

        if self.compute_alignment:
            self.alignment_layer = self.decoder.alignment_layer
            self.alignment_layer.out = self.alignment_layer.out*self.y_mask.dimshuffle(0, 'x', 1)
    ...
    def create_next_probs_computer(self):
        ...
        if not hasattr(self, 'next_probs_fn'):
            fn = self.decoder.build_next_probs_predictor(
                        self.c, self.step_num, self.gen_y, self.current_states)
            self.next_probs_fn = theano.function(
                    inputs=[self.c, self.step_num, self.gen_y] + self.current_states,
                    outputs=[fn[0], fn[1]],
    ...
    ###################################################
    # by He Wei
    def create_next_align_computer(self):
        self.next_align_fn = theano.function(inputs=[],
                                             outputs=[self.decoder.build_next_align_predictor()],
                                             name="next_align_fn", on_unused_input='warn')
        return self.next_align_fn
    ###################################################
...
class BiEncoderDecoder(object):
    ...
    def build(self):
        ...
        self.y = TT.lmatrix('y')
        self.y_mask = TT.matrix('y_mask')
        self.x_1 = TT.lmatrix('x_1')
        self.x_mask_1 = TT.matrix('x_mask_1')
        self.y_1 = TT.lmatrix('y_1')
        self.y_mask_1 = TT.matrix('y_mask_1')
        self.inputs = [self.x, self.y, self.x_mask, self.y_mask, self.x_1, 
                        self.x_mask_1, self.y_1, self.y_mask_1]



        #create shared word embedding
        ...
        shared_embedding = MultiLayer(
            self.rng,
            n_in=self.state['shared_number'],
            n_hids=[self.state['rank_n_approx']],
            activation=[self.state['rank_n_activ']],
            name='shared_embedding',
            **default_kwargs)
        shared_embedding = shared_embedding.W_ems[0]


        self.enc_dec = RNNEncoderDecoder(self.state, self.rng, prefix="", 
                skip_init=self.skip_init, compute_alignment=self.compute_alignment)
        

        self.inv_enc_dec = RNNEncoderDecoder(self.state, self.rng, prefix="inverse_",
                skip_init=self.skip_init, compute_alignment=self.compute_alignment)
        
        self.enc_dec.build(self.x, self.x_mask, self.y, self.y_mask, shared_X=None, shared_Y=shared_embedding)
        self.inv_enc_dec.build(self.x_1, self.x_mask_1, self.y_1, self.y_mask_1, shared_X=shared_embedding, shared_Y=None)

        #self.agree_layer = AgreeLayer("Bidirection RNN agreed")
        #self.agreement = self.agree_layer.train(align_matrix=self.enc_dec.alignment_layer,align_matrix_inv=self.inv_enc_dec.alignment_layer, scale=self.state['agree_beta'])

        logger.debug("Sum enc_dec, inv_enc_dec")
        #attention self.out
        sum_layer = add_cost_layer(self.enc_dec.predictions, self.inv_enc_dec.predictions)
        #sum_layer = add_cost_layer(sum_layer, self.agreement)
        sum_layer.cost = self.enc_dec.predictions.cost + self.inv_enc_dec.predictions.cost #+ self.agreement.cost

        self.sum_layer = sum_layer
    def create_lm_model(self):
        if hasattr(self, 'lm_model'):
            return self.lm_model
        self.lm_model = Multi_LM_Model(
            sum_layer = self.sum_layer,
            cost_layer=[self.enc_dec.predictions, self.inv_enc_dec.predictions],
            sample_fn=[self.enc_dec.create_sampler(), self.inv_enc_dec.create_sampler()],
            weight_noise_amount=self.state['weight_noise_amount'],
            indx_word=self.state['indx_word_target'],
            indx_word_src=self.state['indx_word'],
            indx_word_1=self.state['indx_word_target_1'],
            indx_word_src_1=self.state['indx_word_1'],
            rng=self.rng)
        self.lm_model.load_dict(self.state)
        logger.debug("Model params:\n{}".format(
            pprint.pformat(sorted([p.name for p in self.lm_model.params]))))
...
def add_cost_layer(layer0, other):
    assert hasattr(layer0, 'out'), 'all layers need a default output'
    new_obj = utils.copy(layer0)
...
class AgreeLayer(Layer):
    def __init__(self,name=None):
        self.name = name
        super(AgreeLayer, self).__init__(0, 0, None, self.name)
    #align_matrix shape (target_seq_len, src_seq_len, batch_size)
    #c shape (max_seq_len, batch_size, dim)
    def fprop(self, align_matrix, align_matrix_inv, mode=0):
        inv_align = align_matrix_inv.dimshuffle(1,0,2)
        self.out = align_matrix*inv_align
        return self.out


    def get_cost(self, align_matrix, align_matrix_inv, mode=0, scale=None):
        cost = self.fprop(align_matrix, align_matrix_inv, mode)
        ...
        self.cost = cost*scale
        return self.cost

    def get_grads(self, align_matrix, align_matrix_inv, mode=0, scale=None):
        cost = self.get_cost(align_matrix, align_matrix_inv, mode, scale)
        logger.debug("AgreeLayer caculate gradient")
        grads = TT.grad(cost, self.params)
        logger.debug("AgreeLayer gradient is finished")
